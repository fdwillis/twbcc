...
class Oanda < ApplicationRecord
  ...
  def self.closePosition(token, accountID, tvData, tradeX, tradeInfo, reduceOrKill)
    ...
    if requestProfit.present? && !requestProfit.empty?&& !requestProfit.nil?
      tradeX.take_profits.create!(ticker: tvData['ticker'], profitLoss: tvData['direction'] == 'buy' ?  requestProfit['longOrderFillTransaction']['pl'] : requestProfit['shortOrderFillTransaction']['pl'], traderID: tvData['traderID'], uuid: tvData['direction'] == 'buy' ?  requestProfit['longOrderFillTransaction']['id'] : requestProfit['shortOrderFillTransaction']['id'], status: 'closed', direction: tvData['direction'], broker: tvData['broker'], user_id: User.find_by(oandaToken: token).id)
     end
