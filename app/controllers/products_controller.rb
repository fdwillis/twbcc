...
class ProductsController < ApplicationController
  ...
  def amazon
    ...
    redirect_to "https://www.#{User::ACCEPTEDcountries[params['country'].downcase][:site]}/dp/product/#{params['asin']}?&tag=#{(current_user&.present? && !current_user.referredBy.nil?) ? User.find_by(uuid: current_user&.referredBy).amazonUUID : params['referredBy'].present? ? User.find_by(uuid: params['referredBy']).amazonUUID :  ENV['usAmazonTag']}"
  end
